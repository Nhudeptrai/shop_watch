...
<?php
  $brandid = "";
  // Kiểm tra nếu có tồn tại 'brandid'
  ...
  $product_new = $pd->filterproduct_new($brandid, $catid, $search, $min, $max);
?>
  ...
  <body>
    ...
    <main class="mx-10 my-6 grid grid-cols-[320px_1fr] gap-x-4 product-page">
      ...
      <aside class="bg-red-50 text-red-700 p-3">
        ...
        <form method="GET" action="productbybrand.php" id="filter-form">
          <input type="hidden" name="brandid" value="<?= htmlspecialchars($brandid) ?>">
      ...
      <div>
        ...
        <h1 class="text-red-700 font-bold text-3xl text-left "> Lọc thương hiệu</h1>
          <select id="brand-select" class="w-64 border border-red-200 rounded-2xl px-3 py-2 bg-white text-lg text-red-700 focus:ring-2 focus:ring-red-700">
            ...
            ?>
            <option value="<?= $result_allbrand['brandId'] ?>" <?php if ($brandid == $result_allbrand['brandId']) echo 'selected'; ?>>
    ...
    <script>
      ...
      brandSelect.addEventListener('change', () => {
        ...
        params.delete('brandid'); // Xóa brandid cũ
        let url;
        if (brandId) {
          url = `productbybrand.php?brandid=${brandId}`;
